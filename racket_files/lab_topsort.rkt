...
;; 5 PTS
(define (find-top G)
  (let ([verts (get-vertices G)])
    
    (if (null? verts)
        (topsort-error)
        (if (equal? 0 (indegree (get-edges G) (car verts)))
            (car verts)
            (find-top (make-graph (popv-vertices verts (car verts)) (get-edges G)))
            )
      )
  )
)
...
(define (topsort G)
  (if (null? (get-vertices G))
      '()
      (cons (find-top G) (topsort (pop-vertex G (find-top G))))
  )
)