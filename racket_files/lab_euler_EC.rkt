...
;; 3 PTS
(define (all-even-helper? G vertices)
  (if (null? vertices)
      #t
      (if (odd? (degree G (car vertices)))
         #f
         (all-even-helper? G (cdr vertices))
       )
  )
)
...
(define (adjacent-helper G v)
  (if (null? G)
      '()
      (if (not (member v (car G)))
          (adjacent-helper (cdr G) v)
          (cons
             (if (equal? (caar G) v) (cadar G) (caar G))
             (adjacent-helper (cdr G) v)
          )
       )
   )
...
(define (connected? G)
  (if (null? G)
      #t
      (equal? (usort (dfs G (car (get-vertices G)))) (get-vertices G))
  )
)
